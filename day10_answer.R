...
ggplot( df, aes( x = Var1, y = Freq ) ) + 
  geom_bar( stat = "identity", width = 0.7, fill = "steelblue" ) +              # 막대그래프 에서 y축에  값의 빈도수 만큼 표현 하라 
  ggtitle( "기어의 수" ) + 
...
#문3) 
#R에서 제공하는 mtcars 데이터셋에서 mpg(연비)에 대해 구간 간격이 5.0           # 연속형은 테이블로 만들고 히스토 만들기 힘들다. 그래서 연속형은 구간 간격을 설정해 주어야 ( 범주형일때 테이블 만들고 히스토 )
...
ggplot( data = mtcars, aes( x = mpg, y = wt, color = gear ) ) +              # 기어가 범주형이니 기어수에 따라 색이 다르게 나옴 
  geom_point( size = 3 ) +
  ggtitle( "연비와 중량" ) +
  theme( plot.title = element_text( size = 25, face = "bold", colour = "steelblue" ) )
...
ggplot( data = mtcars, aes( y = mpg, fill = factor( mtcars$cyl ) ) ) +           # 상자 수염 그래프니까 x 축은 큰 의미가 없다. 
  geom_boxplot() +
